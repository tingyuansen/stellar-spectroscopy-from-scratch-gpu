...
**NEW STYLE DIRECTIVE (apply in the QA pass before freeze):** the user finds the code cells "very packed" — prefer **comments on their own line ABOVE the code (interleaved)** over trailing side-comments, and add **blank-line spacing** within cells where suitable, for readability. Comments/blank lines don't change execution → benchmarks stay byte-identical. Apply across ALL 13 lectures.

**CLOSING (task #12):** after L13 — (1) comments-above-code restyle + spacing across all 13; (2) self-review (pedagogy/comments/markdown-explains-code/logical-flow/no-redundancy); (3) run critics (GPT-5.5 + Gemini-3.1-pro) once more, prose fixes; (4) iterate 1-2×; (5) update bible/memory/README; (6) push; **then the textbook is FROZEN.**

**NEXT PROJECT (only AFTER textbook frozen + tested):** GPU/MPS reimplementation of pykurucz in a SEPARATE dir `~/pykurucz_gpu` (plan: `~/pykurucz_gpu/PLAN.md`; memory: pykurucz-gpu-optimization). Target <10s full optical solar spectrum (≥10× the 8-core baseline), PyTorch+MPS on the M4, everything GPU-resident, part-by-part, accuracy-preserving. The textbook is the porting map — "put pykurucz back together from these basic verified functions." **The optimization must NEVER touch the frozen textbook.**
