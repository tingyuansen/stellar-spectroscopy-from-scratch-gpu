...
**CURRENT STATE (2026-06-24) — BOOK IS NOW 14 LECTURES; ALL coverage-audit GAPS FILLED from scratch (no cheating); final stretch.** Restructured 13→14: NEW **L13 Molecular Chemistry** (coupled NMOLEC equilibrium solver 9.92e-14 + molecular continuous opacity CH/OH/H2-CIA bit-exact), capstone moved to **L14**. Gaps reproduced + integrated + committed: autoionizing/merged line types → L5 (bit-exact), convective overshoot + EOS finite-diff derivatives → L11 (bit-exact), molecular chemistry → new L13. Latest commit `d0a4488`. REMAINING: (1) **L14 capstone → true lean-kurucz END-TO-END** (atmosphere FROM SCRATCH for normal stars + emulator fallback, full synthesis, 4 HR stars, parity vs pykurucz end-to-end) — agent `a661a01d` RUNNING; (2) **coverage RE-AUDIT (#19, the HARD GATE)**; (3) **consolidated FINAL POLISH per lecture** = apply text-critic prose fixes + visual-critic plot fixes (#20: L6 identical-T curves, L9 T-correction sign/ΔT + P=(g/κ)τ label, L3 clipped residual axis, L13/L14 low-g wording) + the AGGRESSIVE restyle — do them TOGETHER per lecture to avoid repeat touches; (4) self-review; (5) update bible/memory/README; (6) push → **FREEZE**; then the GPU optimization (hard-gated on a clean re-audit). ~5GB pykurucz data IS downloaded. The DONE items below (1-3) are HISTORICAL.
Three PENDING items [HISTORICAL — all done]:
