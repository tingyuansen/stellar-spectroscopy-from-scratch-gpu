...
- **The markdown is REWRITTEN, not copied.** The external API rewrites the prose to the GPU edition's narrative — it must not read as numpy prose with torch pasted in. Same physics, same equations, GPU-native framing (the loop→tensor story, the precision budget, the kernel choices). Adhere to this bible's voice throughout.
