...
- **A residual ABOVE the float floor is a real ERROR — FIX it, do NOT just "honestly document" it.** The lecture must reproduce the numpy twin over the FULL computation (every depth/layer), not only the spectrum-forming photosphere. The ONLY acceptable documented residual is a genuine fp32-vs-fp64 float-floor difference. Distinguish carefully: the kgpu/numpy-vs-**pyk** coarse-OS deposit is an accepted *method* approximation; but **GPU-port-vs-numpy-TWIN is the SAME physics in torch — any supra-float-floor gap there is a porting bug to fix.** The GPU textbook must be a faithful, in-parity representation of kgpu's implementation, not an approximation of it.

**COMPLETENESS GATE (hard — added after the truncation incident).** A lecture port is not accepted until it covers the numpy twin's **FULL structure**. This is a gate, exactly like parity, enforced by `port_worker.py` (`completeness_gate` over `parse_structure`):
- **Every numpy-twin section is present**, *including the closing sections* — Synthesis / Summary / Practice (Exercises) / Further Reading. A lecture missing its closers FAILS the gate.
- **Comparable cell count:** the port has **≥ 70 %** of the numpy twin's cell count, unless a deliberate, documented re-scope justifies fewer (e.g. L5 ports the metal scatter-add only; L15/L16 are the fp32-vs-fp64 diagnostics). A short port without that justification is treated as **truncated** and FAILS.
