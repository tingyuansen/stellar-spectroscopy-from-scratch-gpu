#!/usr/bin/env python
"""Lecture 6 — Line Opacity II: The Line List. Reading the full Kurucz atomic
...
#  Title + objectives
# ════════════════════════════════════════════════════════════════════════════
md(r"""# Lecture 6 — Line Opacity II: The Line List
...
- `full_lines_data.npz` — the **line catalog**: every atomic line in the 500–510 nm window (wavelength, `log gf`, species, excitation energy, the three damping constants, the line-type code), plus the Voigt **Harris tables** `h0tab/h1tab/h2tab` (the same ones Lecture 5 used), the helium continuum-merge taper limits, and `gt_ahline`, the hydrogen component we subtract to isolate the atomic benchmark.
...
- **`log gf`** — the base-ten log of oscillator strength times lower-level statistical weight, the line's intrinsic strength (Lecture 5);
...
Our reference uses $R \approx 300{,}000$, a spacing of $\sim17\ \mathrm{m\AA}$ at $505\ \mathrm{nm}$ — fine enough to resolve the Doppler cores (Lecture 5) of even the narrowest metal lines. We adopt the reference grid so we can compare point by point.""")
...
Lecture 5 built one Voigt profile with `scipy`'s complex error function. The production code instead evaluates the Voigt function $H(a,v)$ from Kurucz's **Harris polynomial tables** `h0tab/h1tab/h2tab` (Harris 1948), with three branches:
...
These tables ship in the catalog (identical to the ones Lecture 5's reference used). We reproduce the exact branch logic because the accumulation below calls it hundreds of times per line, and any deviation accumulates.""")
