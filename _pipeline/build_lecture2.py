...
code(r'''# Debye lowering per unit charge, Delta_chi/i [eV], at each layer (converged reference value)
potlow = REF["potlow"]
...
code(r'''# hydrogen vs metal share of the electrons across depth
...
md(r"""**The benchmark.** Now — and only now — we load the comparison target from the separate `pfsaha_truth.npz` and check our from-scratch per-ion populations against pykurucz's PFSAHA. This is the payoff: where the temperature-only $U$ missed the metals by up to $27\%$, the full on-the-fly assembly reproduces every per-ion population **to the bit**.""")
