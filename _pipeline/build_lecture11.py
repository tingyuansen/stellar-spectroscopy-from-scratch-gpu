...
# ── RADIAP finalize: the radiation pressure + surface K-moment (computed, not read) ──
md(r"""## Closing the radiation-pressure moment, surface and all
...
Mixing-length theory needs four thermodynamic derivatives at each layer: how the internal energy and the mass density respond to temperature and to pressure, $(\partial E/\partial T)_P$, $(\partial\rho/\partial T)_P$, $(\partial E/\partial P)_T$, $(\partial\rho/\partial P)_T$. From these four the convection kernel constructs the effective heat capacities `heatcp`/`heatcv` — each a combination of all four derivatives, not just $(\partial E/\partial T)_P$, since $c_P$ and $c_V$ must account for the expansion work and the way the density responds to $T$ and $P$. Here `edens` is ATLAS's **specific internal energy** (energy per unit mass, erg/g) and `rho` is the mass density (g/cm$^3$). In a partially-ionized gas these have **no simple closed form** — they depend on the full Saha/Boltzmann equilibrium of Lecture 2: as $T$ rises a little, hydrogen ionizes a little more, the electron count changes, the mean molecular weight changes, and the density and internal energy follow in a way no single $\gamma$ captures. ATLAS therefore computes them by **finite differences**: it re-runs the equation of state at $T\pm0.1\%$ and at $P\pm0.1\%$, reads off `rho` and `edens` at each perturbed point, and central-differences them directly into the four derivatives `CONVEC` needs.
...
# ── forward look ─────────────────────────────────────────────────────────────
md(r"""## What comes next: molecules, the end-to-end spectrum, and the real line-blanketed Sun
...
The book then completes the **synthesis** half — given an atmosphere, the emergent spectrum — in **Lecture 14 (A Spectrum from an Atmosphere, End to End)**, which assembles the whole opacity-and-transfer stack (EOS, continuum, atomic and hydrogen and helium lines, molecular bands, JOSH transfer) and runs it across four stars spanning the HR diagram, computing every spectrum from scratch on the supplied atmosphere state.
