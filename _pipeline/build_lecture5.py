...
w0, w1, w_res = lt_ref["window"]                         # window: start nm, end nm, grid R
n_auto = int(lt_ref["auto_n"])                           # number of autoionizing (line,depth) records
n_cont = int(lt_ref["cont_n"])                           # number of merged-continuum records
...
def autoionizing_delta(wl, cont, slice_lo, center_index, line_wavelength,
    ...
    c = center_index - slice_lo                          # line center, as a slice-local index

    # whole-line gate: drop the line if its peak cannot clear the continuum cutoff
    if kappa0 < cont[c] * cutoff or kappa0 <= 0.0:
        return delta

    # the shore constants ride in the damping slots; gamma is the autoionization width
    freq_center = C_LIGHT_CM / (line_wavelength * NM_TO_CM)   # nu_0 [Hz]
    gamma  = max(abs(gamma_rad), 1e-30)                  # autoionization-broadened width
    ashore = gamma_stark                                 # asymmetry constant (numerator slope)
    bshore = gamma_vdw if abs(gamma_vdw) >= 1e-30 else 1e-30  # floored normalisation constant

    # the center deposits the peak strength unconditionally (past the gate)
    delta[c] += kappa0
    ...
    for offset in range(1, max_steps + 1):
        ...
        for active, idx in ((red_active, c + offset), (blue_active, c - offset)):
            if not active or idx < 0 or idx >= n:
                if idx < 0 or idx >= n:                  # ran off this end of the slice
                    if idx >= n: red_active = False
                    else:        blue_active = False
                continue
            freq  = C_LIGHT_CM / (wl[idx] * NM_TO_CM)    # nu at this grid point [Hz]
            eps   = 2.0 * (freq - freq_center) / gamma   # dimensionless detuning
            value = kappa0 * (ashore * eps + bshore) / (eps * eps + 1.0) / bshore
            if value <= 0.0 or value < cont[idx] * cutoff:   # below cutoff: stop this wing
...
for i in range(n_auto):
    slice_lo = int(lt_ref[f"auto{i}_slice_lo"])          # global index of the slice's first point
    wl   = lt_ref[f"auto{i}_wl_slice"]                    # wavelength slice [nm]
    cont = lt_ref[f"auto{i}_cont_slice"]                  # continuum slice (sets the cutoff)
    ref  = lt_ref[f"auto{i}_delta_vals"]                  # pykurucz's deposited delta (target only)
    ...
    auto_bit += int(np.array_equal(mine, ref))           # exact-equality, not a tolerance
...
code(r'''def merged_continuum_delta(wl, cont, slice_lo, line_wavelength, kappa, cutoff,
    ...
    n = wl.size
    delta = np.zeros(n, dtype=np.float64)
    if kappa <= 0.0 or idx_tail_g <= idx_start_g:        # nothing to deposit
        return delta
    ...
    for idx_g in range(idx_start_g, idx_tail_g):
        loc = idx_g - slice_lo                           # global -> slice-local position
        if loc < 0 or loc >= n:                          # outside the stored slice: pre-merge flat
            continue
        if wl[loc] < line_wavelength:                    # blueward of center: skip
            continue
        value = kappa
        if idx_g >= idx_merge_g:                          # past the merge point: apply the ramp
            value *= max(idx_tail_g - idx_g, 0) / denom
        if value < cont[loc] * cutoff:                   # below cutoff: stop the walk
            break
        delta[loc] += value
    return delta
print("merged-continuum ramp profile ready")''')

md(r"""**Benchmark the merged-continuum profile.** Same protocol: read each record's inputs — the line wavelength, the flat strength $\kappa$, the cutoff, and the three global indices — recompute the ramp, and compare to pykurucz's recorded delta with `np.array_equal`. These are the He I / He II / H I (and a couple of Al I) series limits merging into their continua; we expect all $165$ records bit-exact.""")
...
for i in range(n_cont):
    ...
    wl   = lt_ref[f"cont{i}_wl_slice"]                    # wavelength slice [nm]
    cnt  = lt_ref[f"cont{i}_cont_slice"]                  # continuum slice (sets the cutoff)
    ref  = lt_ref[f"cont{i}_delta_vals"]                  # pykurucz's deposited delta (target only)
...
ai = 0                                                   # an Al I autoionizing record
...
mi = 0                                                   # a merged-continuum record
...
fig, (axa, axm) = plt.subplots(1, 2, figsize=(11, 4.0))
...
axm.set_xlabel("wavelength  [nm]"); axm.set_ylabel(r"line opacity  [cm$^2$/g]")
