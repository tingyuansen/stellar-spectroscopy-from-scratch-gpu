...
*Every result in this book is checked against reference values computed with [**pykurucz**](https://arxiv.org/abs/2603.11693) — a pure-Python implementation of Kurucz's ATLAS12 and SYNTHE — and shipped beside the lectures as small data files, so the notebooks need only NumPy to run. Where we implement the same algorithm the agreement is bit-for-bit; where we take a deliberate shortcut we say so and quantify the residual.*
...
- Build a **grey model atmosphere** — the run of temperature, gas pressure, and column mass with depth — from nothing but $T_{\rm eff}$ and $\log g$, and check it against reference values layer by layer.
...
# the precomputed reference values for this lecture
...
def compare(name, ours, ref, tol=1e-6):
    ...
    # use |ref| as the scale, falling back to 1 where ref is exactly zero
...
lam_nm = np.linspace(200, 2000, 400)
lam_cm = lam_nm * 1e-7

for T in (4500, 5770, 7500):

    # B_nu at each wavelength, then converted to per-wavelength via B_lambda = B_nu * c / lambda^2
    Bnu = planck_nu(C / lam_cm, T)
    Blam = Bnu * C / lam_cm**2
...
code(r'''# same Hopf fit, same optical-depth grid -> both should match the reference exactly
compare("grey tau grid", tau, REF["grey_tau"])
...
You built the scaffolding for everything that follows. You fixed the unit system and the physical constants, and you reproduced the **Planck function** to the bit — the thermal source function that will turn a temperature into a spectrum. You learned that depth is measured in **optical depth**, that the emergent flux forms near $\tau\approx2/3$, and that **LTE** lets the local temperature set the state of the gas. And you built a **grey model atmosphere** of the Sun — $T(\tau)$, $P_{\rm gas}(\tau)$, and the column mass $m(\tau)$ — from $T_{\rm eff}$ and $\log g$ alone, matching the reference layer by layer (the temperature to the bit; the cold-start pressure and column mass to one part in $10^5$).
...
- We work in **CGS** with the reference's exact constants. Where we implement the same algorithm we match bit-for-bit; where we use a simpler shortcut (the cold-start pressure integral) we match to $\sim10^{-5}$ and defer exact integrator matching to Lecture 9.
- The **Planck function** $B_\nu = (2h/c^2)\nu^3/(e^{h\nu/kT}-1)$ is the LTE thermal source function; the overflow-safe Kurucz form reproduces the reference to machine precision.
