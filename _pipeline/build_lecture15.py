...
We import only NumPy and Matplotlib. The benchmark target is `reference/lineblanket_ref.npz`, produced once by the production code on the converged model atmosphere `sun.npz` (surface $T=3696$ K, base $T=11425$ K, base column mass $\rho x = 12.14$ g cm$^{-2}$). Throughout this lecture, *"the real Sun"* is shorthand for this **reference 1D, LTE, line-blanketed Kurucz solar model** at solar parameters — the target a model atmosphere should reproduce — not the actual 3D, time-dependent, partly non-LTE Sun. Like Lecture 11, the file is honest about what it ships as a *given* versus what we *compute* here.
...
where the inner maximum is taken over depth (a line is kept if it beats the continuum at the layer where it is *strongest relative to the continuum*), and the second test re-checks it at the deepest, hottest layer after the Boltzmann excitation factor. A line below this threshold is not physically zero — many weak lines can still add to blanketing in aggregate — but this is the ATLAS/SYNTHE selection approximation that decides which line centers and wings are worth depositing explicitly. There is no wavelength cut inside `SELECTLINES`; the window enters only through which frequency bin each line lands in. The result for the Sun is the ~18-million-line set the production deposit walks; the reference ships the small window subset we teach on.
